...
{
  NS_LOG_FUNCTION (this);
  //NS_LOG_UNCOND("LteEnbNetDevice::DoInitialize::m_componentCarrierManager=" << m_componentCarrierManager << ", m_rrc=" << m_rrc << ", m_handoverAlgorithm" << m_handoverAlgorithm);
  ...
  */
  ...
  for (it = m_ccMap.begin (); it != m_ccMap.end (); ++it)
    {
       ...
       it->second->GetFfrAlgorithm ()->Initialize ();
    }
  // m_mac->Initialize ();
  m_rrc->Initialize ();
  m_handoverAlgorithm->Initialize ();
  ...
  // m_ffrAlgorithm->Initialize ();
  m_componentCarrierManager->Initialize (); //ncrash1
}
