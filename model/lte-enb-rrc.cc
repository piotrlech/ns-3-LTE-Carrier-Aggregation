...
UeManager::DoInitialize ()
{
  ...
  // setup the eNB side of SRB0
  {
    ...
    LteMacSapUser* MacSapUserForMac = m_rrc->m_ccmRrcSapProvider->ConfigureSignalBearer(lcinfo, rlc->GetLteMacSapUser ()); 
    // Signal Channel are only on Primary Carrier
    m_rrc->m_cmacSapProvider.at (0)->AddLc (lcinfo, MacSapUserForMac);
  }
  ...
  switch (m_state)
   ...
   m_caSupportConfigured =  true; //TO Change
}
  ...
  for (itLcOnCcMapping = lcOnCcMapping.begin (); itLcOnCcMapping != lcOnCcMapping.end (); ++itLcOnCcMapping)
    {
      ...
      uint16_t index = itLcOnCcMapping->componentCarrierId;
...
UeManager::ReleaseDataRadioBearer (uint8_t drbid)
{
  ...
  m_drbMap.erase (it);
  std::vector<uint16_t> ccToRelease = m_rrc->m_ccmRrcSapProvider->ReleaseDataRadioBearer (m_rnti, lcid);
  std::vector<uint16_t>::iterator itCcToRelease = ccToRelease.begin ();
...
UeManager::RecvRrcConnectionReconfigurationCompleted (LteRrcSap::RrcConnectionReconfigurationCompleted msg)
{
 ...
 for (uint16_t i = 0; i < m_rrc->m_numberOfComponentCarriers; i++)
          ...
          for (uint16_t i = 0; i < m_rrc->m_numberOfComponentCarriers; i++)
            {
              NS_LOG_DEBUG (this << " Number of ComponentCarriers " << m_rrc->m_numberOfComponentCarriers);
...
UeManager::BuildRrcConnectionReconfiguration ()
{
  ...
  if ( m_caSupportConfigured == false )
    {
      m_caSupportConfigured = true;
      NS_LOG_FUNCTION ( this << "No Ca Configured! Configure NOW!" );
  ...
  for ( uint16_t i = 0; i < m_cphySapUser.size (); i++)
    {
      ...
      m_cmacSapUser[i] = new EnbRrcMemberLteEnbCmacSapUser (this);
      m_ffrRrcSapUser[i] = new MemberLteFfrRrcSapUser<LteEnbRrc> (this);
    }
}
...
LteEnbRrc::~LteEnbRrc ()
{
  //NS_LOG_UNCOND("LteEnbRrc::~LteEnbRrc::m_numberOfComponentCarriers=" << m_numberOfComponentCarriers);
  NS_LOG_FUNCTION (this);
}
...
{
  //NS_LOG_UNCOND("LteEnbRrc::InitializeSap::m_numberOfComponentCarriers=" << m_numberOfComponentCarriers);
  m_cphySapUser.resize (m_numberOfComponentCarriers);
  m_cphySapProvider.resize (m_numberOfComponentCarriers);
  m_cmacSapUser.resize (m_numberOfComponentCarriers);  
  m_cmacSapProvider.resize (m_numberOfComponentCarriers);
  m_ffrRrcSapProvider.resize (m_numberOfComponentCarriers);
}
void
...
void
LteEnbRrc::SetLteEnbCmacSapProvider (LteEnbCmacSapProvider * s, uint16_t pos)
{
  NS_LOG_FUNCTION (this << s);
...
LteEnbCmacSapUser*
LteEnbRrc::GetLteEnbCmacSapUser (uint16_t pos)
{
  NS_LOG_FUNCTION (this);
...
void
LteEnbRrc::SetLteFfrRrcSapProvider (LteFfrRrcSapProvider * s, uint16_t index)
{
  NS_LOG_FUNCTION (this << s);
...
void
LteEnbRrc::ConfigureCell (std::map<uint8_t, Ptr<ComponentCarrierEnb> > ccm, uint16_t cellId)
{
...
{
  NS_LOG_FUNCTION (this);
  //NS_LOG_UNCOND("LteEnbRrc::DoAllocateTemporaryCellRnti::m_numberOfComponentCarriers" << m_numberOfComponentCarriers);
