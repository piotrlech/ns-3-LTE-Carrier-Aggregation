...
: m_ccmRrcSapUser (0)
{
  //NS_LOG_UNCOND ("SimpleUeComponentCarrierManager Created " << this);
...
SimpleUeComponentCarrierManager::~SimpleUeComponentCarrierManager ()
{
  //NS_LOG_UNCOND ("SimpleUeComponentCarrierManager Deleted " << this);
}
...
{
  //NS_LOG_UNCOND ("SimpleUeComponentCarrierManager Disposed " << this);
...
SimpleUeComponentCarrierManager::DoTransmitPdu (LteMacSapProvider::TransmitPduParameters params)
{
  ...
  NS_ASSERT_MSG (it != m_CcMacSapProvider.end (), "could not find Sap for ComponentCarrier " << (uint16_t) params.componentCarrierId);
...
SimpleUeComponentCarrierManager::DoRemoveLc (uint8_t lcid)
{
  ...
  std::map<uint16_t, std::map<uint8_t, LteMacSapProvider*> >::iterator it =  m_componentCarrierLcMap.begin ();
...
SimpleUeComponentCarrierManager::DoAddLc (uint8_t lcId,  LteUeCmacSapProvider::LogicalChannelConfig lcConfig, LteMacSapUser* msu)
{
  ...
  std::map <uint16_t, std::map<uint8_t, LteMacSapProvider*> >::iterator ccLcMapIt;
  for (uint16_t ncc = 0; ncc < m_noOfComponentCarriersEnabled; ncc++)
    {
      ...
      if (ccLcMapIt != m_componentCarrierLcMap.end ())
        {
          ccLcMapIt->second.insert (std::pair <uint8_t, LteMacSapProvider*> (lcId, m_CcMacSapProvider.at (ncc)));
        }
      else
        {
          std::map<uint8_t, LteMacSapProvider*> empty;
          std::pair <std::map <uint16_t, std::map<uint8_t, LteMacSapProvider*> >::iterator, bool> 
            ret = m_componentCarrierLcMap.insert (std::pair <uint16_t,  std::map<uint8_t, LteMacSapProvider*> > 
                                                  (ncc, empty));
  ...
  if (m_noOfComponentCarriersEnabled < m_noOfComponentCarriers)
   {
     ...
     //here the code to update all the Lc, since now  those should be mapped on all ComponentCarrier
...
void
SimpleUeComponentCarrierManager::DoAmIalive ()
{
  //NS_LOG_UNCOND ("SimpleUeComponentCarrierManager::DoAmIalive=" << this);
}
...
SimpleUeComponentCarrierManager::DoConfigureSignalBearer (uint8_t lcid,  LteUeCmacSapProvider::LogicalChannelConfig lcConfig, LteMacSapUser* msu)
{
  NS_LOG_FUNCTION (this);
  // std::map<uint8_t, LteMacSapUser*>::iterator it = m_lcAttached.find (lcid);
  // NS_ASSERT_MSG (it == m_lcAttached.end (), "Warning, LCID " << (uint16_t) lcid << " already exist");
  m_lcAttached.insert (std::pair<uint8_t, LteMacSapUser*> (lcid, msu));
