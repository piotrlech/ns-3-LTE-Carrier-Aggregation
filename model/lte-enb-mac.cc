...
m_ulCcmMacSapUser (0)
{
  ...
  m_ulCcmMacSapProvider = new MemberLteUlCcmMacSapProvider<LteEnbMac> (this);
}
...
{
  return m_cmacSapProvider;//crash0
}
...
void
LteEnbMac::SetLteUlCcmMacSapUser (LteUlCcmMacSapUser* s)
{
  m_ulCcmMacSapUser = s;
}


LteUlCcmMacSapProvider*
LteEnbMac::GetLteUlCcmMacSapProvider ()
{
  return m_ulCcmMacSapProvider;
}
  ...
  if (m_ulCeReceived.size () > 0)
    {
      ...
      NS_LOG_DEBUG (this << " bsr Size Before erasing in SubFrameIndication Function " << (uint16_t) m_ulCeReceived.size ());
      NS_LOG_DEBUG (this << "CHECK ulMacReq.m_macCeList size SubFrameIndication Function " << (uint16_t) ulMacReq.m_macCeList.size ());
