...
void
LteUeNetDevice::test (void)
{
  //NS_LOG_UNCOND ("LteUeNetDevice::test=" << this);
  Ptr<LteUeRrc> ueRrc = this->GetRrc ();             // good
  Ptr<LteUeRrc> rrc = this->GetObject<LteUeRrc> ();  // bad
  //NS_LOG_UNCOND("LteUeNetDevice::test::rrc=" << ueRrc << ", " << rrc << ", " << m_rrc);
  m_rrc->test();  //2crash-bad
}
