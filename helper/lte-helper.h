 ...
 *     * CCS (Carrier Component Selection) algorithm
...
public:
 ...
 /**
   ...
   */
  void SetEnbCc (std::map< uint8_t, Ptr<ComponentCarrierEnb> > ccmap);
   ...
   *
   * \param type type of carrier component algorithm, must be a type name of any class
   *             inheriting from ns3::LteCcsAlgorithm, for example:
   *             "ns3::NoOpCcsAlgorithm"
   *
   * Equivalent with setting the `CcsAlgorithm` attribute.
   */
   ...
   *
   * \param type type of Component Carrier Manager, must be a type name of any class
   *             inheriting from ns3::LteCcsAlgorithm, for example:
   *             "ns3::NoOpCcsAlgorithm"
   *
   * Equivalent with setting the `CcsAlgorithm` attribute.
...
private:
  ...
  void ChannelModelInitialized (void);

  /**
   * \brief This function just create the enb ComponentCarrierMap when CA is not enabled
   */
  void DoCreateEnbComponentCarrierMap (uint16_t ulearfc, uint16_t dlearfcn, uint8_t ulbw, uint8_t dlbw);
   ...
   */
  std::map< uint8_t, Ptr<ComponentCarrierEnb> > m_enbComponentCarrierMap;
