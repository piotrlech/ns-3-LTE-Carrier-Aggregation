...
This consists of the possibility, for a single user equipment (UE), to aggregate Radio Resources belonging to different carriers, in order to have more bandwidth available, and a higher potential throughput.

3GPP defines three different CA bandwidth class for releases 10 and 11 (where ATBC is Aggregated Transmission Bandwidth Configuration):
...
The impact of this technique on the different layer of LTE protocol stack is shown in the figure :ref:`fig-lte-carrier-aggregation-impact` (from 3gpp.org).
...
The main impact on the RRC layer is the modification of the RRC Connection Reconfiguration message, basically through this message, the EnB RRC layer sends the parameters to configure the Second Component Carriers to the Ue RRC layer.
This procedure starts when the Ue is in the CONNECTED_NORMALLY state (see the RRC state machine description). The Carrier Aggregation enabling procedure is shown in figure :ref:`lte-ca-enable-procedure`. In this implementation, it was decided to simplify the procedure, so the Ue Capability Inquiry and Ue Capability Information messages are not implemented; in other words, the hypothesis is that each Ue can support carrier aggregation.

.. _fig-lte-ca-enable-procedure:
 
.. figure:: figures/lte-ca-enable-procedure.*
   :scale: 80 %
   :align: center
...
The impact on MAC layer is quite large. With Carrier Aggregation, MAC layer has to manage the scheduling of data on multiple Component Carriers (CCs). There is one HARQ process for each CC and those are indipendent, while for the scheduling two different schemes are proposed in the R10. Both schemes are shown in figure :ref:`fig-lte-carrier-aggregation-mac-impact` (from 3gpp.org)
...
In the case a) Scheduling grant and resources are on the same carrier, basically one PDCCH per carrier. The traffic is scheduling on one carrier or on the others according to whom notify the transmit opportunity to the RLC layer.

In the case b), cross-carrier scheduling: a new field is added in the PDCCH of the primary carrier (PCC), CIF, which basically help to identify which carriers is currently addressing.

In this first implementation, only the case a) is implemented.
...
.. _fig-lte-net-phy-relation:
 
.. figure:: figures/lte-class-net-phy-relation.*
   :scale: 70 %
   :align: center
